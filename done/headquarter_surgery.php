...
$dbName= "healthcare";
//initialize errors
$aadharerror=$doctoriderror=$primaryiderror=$remarkerror=$opiderror=
$refererror=$verifyerror=$operationerror="";$run=0;
...
if ($_SERVER["REQUEST_METHOD"]=="POST") {
    try {
        if (isset($_POST["ok"])) {
            ...
            if ($patientverify=="") {
                $verifyerror="Please select value";
                $run=1;
            }
            ...
            if ($run==0) {
                ...
                $sql="insert into surgery_details(aadhar_id, operation_remark, operated_verify, operated_doctor_id, doctor_id)"."values(?,?,?,?,?)";
                if ($stmt=mysqli_prepare($con, $sql)) {
                    mysqli_stmt_bind_param($stmt, "isssi", $aadhar, $remark, $patientverify, $operationid, $doctorid);
                ...
                echo '<script type="text/javascript">alert("PLEASE ENTER THE CORRECT DATA");</script>';
            }//end if
...
<br><br>
	...
	<div id="ok">
		<input type="submit" name="ok" value="SUBMIT" style="margin-left:10%;font-size:20px;">
		<input type="submit" name="signout" value="SIGNOUT" style="margin-left:20%;font-size:20px;">
	</div>
	</div></form>
